...
run_interactive() {
    ...
    echo -e "${YELLOW}Working commands in interactive shell:${NC}"
    echo -e "  source milk/milk_env.sh         # Load MILK environment"
    echo -e "  cd milk/examples                # Navigate to examples"
    echo -e "  python3 -c 'import os; print(f\"MAUD: {os.environ.get(\\\"MAUD_PATH\\\")}\")' # Test environment"
    echo -e "  ls -la /opt/maud/               # Check MAUD installation"
    echo -e "  ls -la milk/examples/           # Browse MILK examples"
    echo -e "  cat milk/milk_env.sh            # View environment setup"
    echo -e "  java -version                   # Check Java version"
    echo -e "  python3 --version               # Check Python version"
    echo -e "  exit                            # Exit session"
    echo
    echo -e "${YELLOW}Note: Direct MAUD execution may have limitations in containerized environment.${NC}"
    echo -e "${YELLOW}Use 'test' and 'analysis' options for full MILK workflow demonstrations.${NC}"
    echo
